...
```{r DataExercise-1a}
high_temps_F <- c(52, 34, 41, 49, 56, 65, 69, 69, 66, 62)
low_temps_F <- c(21, 18, 18, 29, 46, 57, 58, 58, 51, 42)
...
high_temps_C <- fahrenheit_to_celsius(high_temps_F)
low_temps_C <- fahrenheit_to_celsius(low_temps_F)
```

Combine your four vectors into a data frame and add informative column names.

```{r DataExercise-1c}
temp.df <- data.frame(high_temps_F, high_temps_C, low_temps_F, low_temps_C)
names(temp.df) <- c("Highs in Fahrenheit", "Highs in Celsius", "Lows in Fahrenheit", "Lows in Celsius")
temp.df
```
